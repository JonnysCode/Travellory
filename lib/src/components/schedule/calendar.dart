  ...
  List<Meeting> _getDataSource(List<TripModel> trips) {
    ...
    for (final trip in trips) {
      final Meeting meeting = Meeting(trip.name, getDateTimeFrom(trip.startDate),
          getDateTimeFrom(trip.endDate), tripColor[index],
          isAllDay: true);
          ...
          child: Container(
            ...
            child: SfCalendar(
              ...
              dataSource: MeetingDataSource(
                  _getDataSource(tripsProvider.trips.map((trip) => trip.tripModel).toList())),
              selectionDecoration: BoxDecoration(
                color: Colors.black12,
                shape: BoxShape.circle,
                boxShadow: [
                  BoxShadow(
                      blurRadius: 2, color: Colors.black.withOpacity(.1), offset: Offset(0.0, 0.0))
                ],
              ),
